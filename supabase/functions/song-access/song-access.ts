// Mirror of the DEPLOYED `song-access` Edge Function (version 1, verify_jwt
// on), captured verbatim from the live project on 2026-07-11. Kept in git so
// the repo records what actually runs; deploys still happen via the dashboard
// or `supabase functions deploy song-access`.
//
// This function is the ONLY path to song audio. The private `song` bucket has
// no select policy, so clients can't read Storage directly; this function
// verifies the caller's JWT, re-runs the server-side distance check
// (get_song_access), and only then mints a 5-minute signed URL using the
// service-role client (which bypasses Storage RLS by design).
